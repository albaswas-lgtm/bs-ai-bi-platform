const CACHE_NAME = 'bs-bi-platform-v2-3-save-ux-fix';
...
function injectSaveExperience(html) {
  if (html.includes('id="bsSaveModal"')) return html;
...
#bsSaveModal.open{display:grid}
.bs-save-card{width:min(520px,94vw);background:linear-gradient(180deg,#1a1f2e,#0f121c);border:1px solid #3b4358;border-radius:22px;padding:22px;box-shadow:0 30px 90px #000c;color:#f7f2e8}
.bs-save-card h3{margin:0 0 6px;color:#e7c15f;font-size:1.3rem}.bs-save-card p{margin:0 0 16px;color:#b8b4ad;font-size:.87rem;line-height:1.8}
.bs-save-options{display:grid;gap:10px}.bs-save-option{width:100%;display:flex;align-items:center;gap:12px;text-align:right;padding:14px;border-radius:15px;border:1px solid #343b50;background:#121724;color:#fff;cursor:pointer;transition:.2s}.bs-save-option:hover{transform:translateY(-1px);border-color:#e7c15f;background:#181e2d}.bs-save-option strong{display:block;color:#fff}.bs-save-option span{display:block;color:#aaa7a1;font-size:.77rem;margin-top:2px}.bs-save-icon{font-size:1.5rem}.bs-save-cancel{margin-top:12px;width:100%;padding:10px;border:0;border-radius:12px;background:#272b3d;color:#fff;font-weight:800;cursor:pointer}
...
(function(){
  function showToast(message){
    var toast=document.getElementById('bsToast');
    toast.textContent=message;toast.classList.add('show');
    clearTimeout(window.__bsToastTimer);window.__bsToastTimer=setTimeout(function(){toast.classList.remove('show')},2600);
  }
  ...
  async function saveBackupFile(){
    var data=JSON.stringify(projectPayload(),null,2);
    var safeName=(project.name||'bs-project').replace(/[^a-zA-Z0-9\u0600-\u06FF_-]+/g,'-');
    var filename=safeName+'-backup.json';
    try{
      if(window.showSaveFilePicker){
        var handle=await window.showSaveFilePicker({suggestedName:filename,types:[{description:'BS Project Backup',accept:{'application/json':['.json']}}]});
        var writable=await handle.createWritable();await writable.write(data);await writable.close();
      }else{
        var blob=new Blob([data],{type:'application/json'});var url=URL.createObjectURL(blob);var a=document.createElement('a');a.href=url;a.download=filename;a.click();setTimeout(function(){URL.revokeObjectURL(url)},1000);
      }
      closeSaveModal();showToast(lang==='ar'?'تم حفظ النسخة الاحتياطية بنجاح':'Backup saved successfully');
    }catch(error){if(error&&error.name!=='AbortError')showToast(lang==='ar'?'تعذر حفظ الملف. حاولي مرة أخرى.':'Could not save the file. Please try again.')}
  }
  window.addEventListener('DOMContentLoaded',function(){
    saveProject=function(){openSaveModal()};
    applyProjectSettings=function(){
      project={name:document.getElementById('projectNameInput').value.trim()||'Project',company:document.getElementById('companyInput').value.trim()||'BS',activity:document.getElementById('activityInput').value,currency:document.getElementById('currencyInput').value};
      localStorage.setItem('bs_bi_project',JSON.stringify(project));
      updateProjectView();updateMetrics();openPage('homePage');showToast(lang==='ar'?'تم تحديث إعدادات المشروع':'Project settings updated');
    };
    ...
    event.respondWith(
      fetch(event.request, {cache: 'no-store'})
        .then(async response => {
          ...
          const enhanced = injectSaveExperience(html);
        ...
        .catch(async () => {
          ...
          return new Response(injectSaveExperience(await cached.text()), {headers:{'Content-Type':'text/html; charset=utf-8'}});
        })
    );
    return;
  }
